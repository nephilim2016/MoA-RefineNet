...
def training_moa(
    ...
    for epoch in range(epochs_moa):
        ...
        if t < 0.4:
            ...
            lambda_sparse = 0.30  # slightly stronger force
            lambda_div = 0.50
        ...
        if (0.6 <= t < 0.8) and ema_H.value:
            if ema_H.value < 0.60:
                ...
                lambda_sparse = max(0.30, float(lambda_sparse))
                lambda_div = min(0.50, float(lambda_div))
...
if __name__ == "__main__":
    ...
    optimizer_moa_part_backbone_moa = tf.keras.optimizers.AdamW(
